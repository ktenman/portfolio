...
## Follow-up: manifest templating (decided 2026-05-27)

Local dev was later moved to **Skaffold + Jib** (PR #1533), which added a
`k8s/overlays/local` kustomize overlay. The repo now has **two templating
mechanisms**: prod still uses `envsubst` on `secrets.yaml`/`backend.yaml`
(`deploy-k3s.yml`); local uses kustomize (`secretGenerator` from `.env.local` +
`backend-patch.yaml`). The prod path is untouched, so the minimum-change parity
goal still holds, but the split is a real maintainability smell.

A Helm migration was proposed and **declined for now** — it reopens the
deliberately-deferred "v2" decision and would change the prod mechanism before
the gated `fov.ee` cutover is proven on live data. The refactor stays deferred
until prod is stable on k3s. When it happens, prefer **kustomize-consolidation**
(`base/` + `overlays/prod` + `overlays/local`, dropping `envsubst`) over Helm:
an 8-service single-node app does not need Helm's Go-templating, and kustomize is
already half-present and built into `kubectl`. Reach for Helm only if real
multi-environment or conditional-rendering needs appear.
