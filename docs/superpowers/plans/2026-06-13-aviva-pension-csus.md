...
- Modify: `src/test/kotlin/ee/tenman/portfolio/ft/HistoricalPricesServiceTest.kt` — add a test that wires a _real_ `CurrencyConversionService` (mocked repo) so FT pounds → EUR is exercised end-to-end.
...
(Note: this is a characterization test — the production code is already correct, verified live. The test exists to _lock_ that correctness, replacing the stub's false confidence.)
