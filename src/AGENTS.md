...
- **Never nest data classes inside service classes** - a DTO or value object declared inside a service is not reusable or testable. Declare it at the top level.
- **Group related top-level declarations in one file** - DTOs, enums, exceptions, value objects and `@ConfigurationProperties` that belong to the same cluster live together (`DiversificationDtos.kt`, `LightyearResponses.kt`), named for the cluster rather than one member. One declaration per file is only required for Spring stereotypes (`@Service`, `@Component`, `@RestController`, `@Configuration`, `@RestControllerAdvice`) and JPA `@Entity` classes. This is enforced by ArchUnit tests.
...
Exact counts drift as features land — discover current state with grep instead of trusting a static list:

- **Controllers**: 11 `@RestController` classes (`grep -rl "@RestController" src/main/kotlin`) plus `HomeController` (a plain `@Controller` serving the Vue SPA). Core CRUD: Instrument, PortfolioTransaction, PortfolioSummary. Analytics: EtfBreakdown, Calculator, DiversificationCalculator. Support: Logo, VehicleInfo, BuildInfo, Health, Enum.
- **Services**: ~76 `@Service` classes (`grep -rl "@Service" src/main/kotlin`), grouped by domain: calculation, etf, pricing, summary, transaction, instrument, logo, vehicle — plus infrastructure (cache, image, job, minio) and integration (country/industry classification).
- **Background jobs**: ~15 `@Scheduled` jobs in the `job` package — price updates (Binance, FT, Trading212, Lightyear), XIRR/analytics, ETF holdings classification/logos/TER.
- **Domain**: 9 JPA `@Entity` classes + 14 enums in the `domain` package (`grep -rl "@Entity" src/main/kotlin`).
...
- `PriceSnapshot` - Intraday latest-price cache per provider - UNIQUE(instrument_id, provider_name, snapshot_hour)
...
- `DiversificationConfig` - Persisted configuration for the diversification calculator
- `JobExecution` - Background job execution tracking with status and error handling

Migrations are in `src/main/resources/db/migration/` using timestamp naming `VYYYYMMDDHHMM__description.sql` (e.g. `V202605261300__lightyear_sells_may25.sql`) — 200+ migrations applied in timestamp order. Name new migrations with the current date/time, not a sequential V-number.

### External Integrations (12 systems)

- **Market Data:** FT (stocks/ETFs via HTML scraping), Binance (crypto via JSON API), OpenFIGI (instrument identifier resolution)
