...
@Service
class VeegoService(
    private val veegoWebClient: VeegoWebClient,
) {
    private val log = LoggerFactory.getLogger(javaClass)

    suspend fun getTaxInfo(plateNumber: String): VeegoResult {
        log.info("Fetching tax info for plate: {}", plateNumber)
        val startTime = System.nanoTime()
        return runCatching {
            val response = veegoWebClient.getTaxInfo(plateNumber)
            val duration = TimeUtility.durationInSeconds(startTime).toDouble()
            log.info(
              "Tax info retrieved for {}: {} EUR annual, {} EUR registration",
              plateNumber,
              response.annualTax,
              response.registrationTax,
            )
