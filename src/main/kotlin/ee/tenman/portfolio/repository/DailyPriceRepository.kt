...
interface DailyPriceRepository : JpaRepository<DailyPrice, Long> {
  ...
  @Query(
    "SELECT dp FROM DailyPrice dp WHERE dp.instrument.id IN :instrumentIds" +
      " AND dp.entryDate BETWEEN :startDate AND :endDate ORDER BY dp.entryDate",
  )
  fun findAllByInstrumentIdInAndEntryDateBetween(
