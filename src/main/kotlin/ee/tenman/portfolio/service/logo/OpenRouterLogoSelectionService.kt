...
class OpenRouterLogoSelectionService(
  ...
  private fun callVisionApi(
    ...
    images.forEachIndexed { index, (_, imageData) ->
      ...
      val mediaType = detectMediaType(imageData)
      contentParts.add(
  ...
  private fun detectMediaType(imageData: ByteArray): String {
    if (imageData.size < 4) return "image/png"
    return when {
      imageData[0] == 0x89.toByte() && imageData[1] == 0x50.toByte() -> "image/png"
      imageData[0] == 0xFF.toByte() && imageData[1] == 0xD8.toByte() -> "image/jpeg"
      imageData[0] == 0x52.toByte() && imageData[1] == 0x49.toByte() -> "image/webp"
      else -> "image/png"
    }
  }
  ...
  companion object {
    ...
    private const val VISION_MODEL = "google/gemini-2.5-flash"
  }
}
