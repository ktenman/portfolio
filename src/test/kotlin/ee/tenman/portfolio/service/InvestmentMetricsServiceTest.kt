...
class InvestmentMetricsServiceTest {
  ...
  @Test
  fun `should holdings with single buy transaction`() {
    val transaction = createBuyTransaction(quantity = BigDecimal("10"), price = BigDecimal("100"))

    val (quantity, averageCost) = investmentMetricsService.holdings(listOf(transaction))
  ...
  @Test
  fun `should holdings with multiple buy transactions`() {
    val transactions =
      listOf(
    ...
    val (quantity, averageCost) = investmentMetricsService.holdings(transactions)
  ...
  @Test
  fun `should holdings with buy and sell transactions`() {
    val transactions =
      listOf(
  ...
  @Test
  fun `should holdings with complete sell-off`() {
    val transactions =
      listOf(
  ...
  @Test
  fun `should holdings handles commission correctly`() {
    val transaction =
      createBuyTransaction(
        quantity = BigDecimal("10"),
        price = BigDecimal("100"),
  ...
  @Test
  fun `should value returns correct value`() {
    val holdings = BigDecimal("100")
    val currentPrice = BigDecimal("150.50")

    val result = investmentMetricsService.value(holdings, currentPrice)

    expect(result).toEqualNumerically(BigDecimal("15050.00"))
  }

  @Test
  fun `should value with zero holdings`() {
    val result = investmentMetricsService.value(BigDecimal.ZERO, BigDecimal("150"))

    expect(result).toEqualNumerically(BigDecimal.ZERO)
  }

  @Test
  fun `should profit returns positive profit`() {
    val holdings = BigDecimal("100")
    val averageCost = BigDecimal("50")
    val currentPrice = BigDecimal("75")

    val profit = investmentMetricsService.profit(holdings, averageCost, currentPrice)

    expect(profit).toEqualNumerically(BigDecimal("2500"))
  }

  @Test
  fun `should profit returns negative profit (loss)`() {
    val holdings = BigDecimal("100")
  ...
  @Test
  fun `should profit returns zero when no price change`() {
    val holdings = BigDecimal("100")
    val price = BigDecimal("50")

    val profit = investmentMetricsService.profit(holdings, price, price)

    expect(profit).toEqualNumerically(BigDecimal.ZERO)
  }

  @Test
  fun `should xirr with buy transactions only`() {
    val transactions =
      listOf(
    ...
    val xirrTransactions = investmentMetricsService.xirr(transactions, currentValue, testDate)
  ...
  @Test
  fun `should xirr with buy and sell transactions`() {
    val transactions =
      listOf(
  ...
  @Test
  fun `should xirr with zero current value`() {
    val transactions = listOf(createBuyTransaction(quantity = BigDecimal("10"), price = BigDecimal("100")))

    val xirrTransactions = investmentMetricsService.xirr(transactions, BigDecimal.ZERO, testDate)
  ...
  @Test
  fun `should adjusted with sufficient transactions returns bounded value`() {
    val transactions =
      listOf(
    ...
    val xirr = investmentMetricsService.adjusted(transactions, testDate)

    expect(xirr).toBeGreaterThanOrEqualTo(-10.0).and.toBeLessThanOrEqualTo(10.0)
  }

  @Test
  fun `should adjusted with fewer than 2 transactions returns zero`() {
  ...
  @Test
  fun `should adjusted applies dampening for new investments`() {
  ...
  @Test
  fun `should adjusted handles empty negative cashflows`() {
    val transactions =
      listOf(
        Transaction(500.0, testDate.minusDays(50)),
  ...
  fun `should adjusted bounds extreme values`(
  ...
  @Test
  fun `should metrics with empty transactions returns empty metrics`() {
    val metrics = investmentMetricsService.metrics(testInstrument, emptyList(), testDate)

    expect(metrics).toEqual(InstrumentMetrics.EMPTY)
  }

  @Test
  fun `should metrics with single platform calculates correctly`() {
    val transactions =
      listOf(
        createBuyTransaction(quantity = BigDecimal("10"), price = BigDecimal("100"), platform = Platform.LHV),
      )

    val metrics = investmentMetricsService.metrics(testInstrument, transactions, testDate)

    expect(metrics.quantity).toEqualNumerically(BigDecimal("10"))
    expect(metrics.investment).toBeGreaterThan(BigDecimal.ZERO)
    expect(metrics.value).toBeGreaterThan(BigDecimal.ZERO)
  }

  @Test
  fun `should metrics with multiple platforms aggregates correctly`() {
    val transactions =
      listOf(
        createBuyTransaction(
          quantity = BigDecimal("10"),
  ...
  @Test
  fun `should metrics with zero price returns zero values`() {
    testInstrument.currentPrice = BigDecimal.ZERO
    val transactions = listOf(createBuyTransaction(quantity = BigDecimal("10"), price = BigDecimal("100")))

    val metrics = investmentMetricsService.metrics(testInstrument, transactions, testDate)

    expect(metrics.value).toEqualNumerically(BigDecimal.ZERO)
    expect(metrics.profit).toBeLessThan(BigDecimal.ZERO)
  }

  @Test
  fun `should metricsWithProfits calls transaction service`() {
    val transactions = listOf(createBuyTransaction(quantity = BigDecimal("10"), price = BigDecimal("100")))

    every { transactionService.calculateProfits(any(), any()) } returns Unit

    val metrics = investmentMetricsService.metricsWithProfits(testInstrument, transactions, testDate)
  ...
  @Test
  fun `should convert for BUY transaction has negative amount`() {
    val buyTransaction = createBuyTransaction(quantity = BigDecimal("10"), price = BigDecimal("100"))

    val xirrTx = investmentMetricsService.convert(buyTransaction)
  ...
  @Test
  fun `should convert for SELL transaction has positive amount`() {
    val sellTransaction = createSellTransaction(quantity = BigDecimal("10"), price = BigDecimal("120"))

    val xirrTx = investmentMetricsService.convert(sellTransaction)
  ...
  @Test
  fun `should convert includes commission in calculation`() {
    val buyTransaction =
      createBuyTransaction(
        quantity = BigDecimal("10"),
        price = BigDecimal("100"),
  ...
  @Test
  fun `should metricsWithProfits with empty transactions returns empty metrics`() {
    val metrics =
  ...
  @Test
  fun `should metricsWithProfits with multiple platforms`() {
    val transactions =
      listOf(
        createBuyTransaction(quantity = BigDecimal("10"), price = BigDecimal("100"), platform = Platform.LHV),
  ...
  @Test
  fun `should metricsWithProfits with all positions sold`() {
    val transactions =
      listOf(
        createBuyTransaction(quantity = BigDecimal("10"), price = BigDecimal("100")),
    ...
    expect(metrics.investment).toEqualNumerically(BigDecimal.ZERO)
  }

  @Test
  fun `should adjusted with very short investment period applies damping`() {
    val transactions =
      listOf(
  ...
  @Test
  fun `should adjusted with investment period over 60 days has full damping`() {
    val transactions =
      listOf(
        Transaction(-1000.0, testDate.minusDays(100)),
        Transaction(1500.0, testDate),
  ...
  @Test
  fun `should adjusted with exactly 60 days investment period`() {
    val transactions =
      listOf(
  ...
  @Test
  fun `should adjusted handles exception and returns zero`() {
    val transactions =
      listOf(
        Transaction(-1000.0, testDate),
        Transaction(1000.0, testDate),
  ...
  @Test
  fun `should xirr with negative current value omits final transaction`() {
    val transactions = listOf(createBuyTransaction(quantity = BigDecimal("10"), price = BigDecimal("100")))

    val xirrTransactions =
      investmentMetricsService.xirr(
        transactions,
        BigDecimal("-100"),
        testDate,
      )
  ...
  @Test
  fun `should xirr with multiple buy and sell transactions`() {
    val transactions =
      listOf(
  ...
  @Test
  fun `should holdings with sell before buy returns zero`() {
    val transactions =
      listOf(
  ...
  @Test
  fun `should holdings with partial sell reduces cost proportionally`() {
    val transactions =
      listOf(
        createBuyTransaction(quantity = BigDecimal("100"), price = BigDecimal("50")),
  ...
  @Test
  fun `should metrics with null current price uses zero`() {
  ...
  @Test
  fun `should metrics with platform having zero holdings is excluded`() {
    val transactions =
      listOf(
        createBuyTransaction(quantity = BigDecimal("10"), price = BigDecimal("100"), platform = Platform.LHV),
  ...
  @Test
  fun `should convert for sell with commission reduces amount`() {
    val sellTransaction =
  ...
  @Test
  fun `should adjusted with weighted investment age calculation`() {
    val transactions =
      listOf(
  ...
  @Test
  fun `should holdings with multiple consecutive sells`() {
    val transactions =
      listOf(
        createBuyTransaction(quantity = BigDecimal("100"), price = BigDecimal("50")),
        createSellTransaction(quantity = BigDecimal("25"), price = BigDecimal("60")),
  ...
  @Test
  fun `should holdings with zero commission`() {
    val transaction =
      createBuyTransaction(
        quantity = BigDecimal("10"),
        price = BigDecimal("100"),
  ...
  @Test
  fun `should holdings with very small quantities`() {
    val transactions =
      listOf(
  ...
  @Test
  fun `should metrics with one platform having zero holdings after sell`() {
    val transactions =
      listOf(
        createBuyTransaction(quantity = BigDecimal("10"), price = BigDecimal("100"), platform = Platform.LHV),
        createSellTransaction(quantity = BigDecimal("10"), price = BigDecimal("120"), platform = Platform.LHV),
  ...
  @Test
  fun `should adjusted with multiple investments at different times has weighted damping`() {
    val transactions =
      listOf(
  ...
  @Test
  fun `should convert with very high commission for buy`() {
    val buyTransaction =
      createBuyTransaction(
  ...
  @Test
  fun `should convert with zero commission for sell`() {
    val sellTransaction =
      createSellTransaction(
        quantity = BigDecimal("10"),
        price = BigDecimal("100"),
  ...
  @Test
  fun `should metrics with negative profit scenario`() {
    ...
    expect(metrics.profit).toBeLessThan(BigDecimal.ZERO)
    expect(metrics.value).toBeLessThan(metrics.investment)
  }
