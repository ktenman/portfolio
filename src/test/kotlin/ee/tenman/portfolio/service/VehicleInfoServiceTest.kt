...
class VehicleInfoServiceTest {
    ...
    fun `should return combined vehicle info when both APIs succeed`() {
        ...
        coEvery { auto24Service.findCarPrice(plateNumber) } returns CarPriceResult(price = "3400 € kuni 8300 €", durationSeconds = 1.5)
        coEvery { veegoService.getTaxInfo(plateNumber) } returns createSuccessfulVeegoResult()
    ...
    fun `should return partial data when auto24 returns vehicle not found`() {
        ...
        coEvery { auto24Service.findCarPrice(plateNumber) } returns
    ...
    fun `should return partial data when veego fails but auto24 succeeds`() {
        val plateNumber = "876BCH"
        coEvery { auto24Service.findCarPrice(plateNumber) } returns CarPriceResult(price = "5000 €", durationSeconds = 1.0)
        coEvery { veegoService.getTaxInfo(plateNumber) } returns VeegoResult.error("Connection refused", 0.1)
    ...
    fun `should return errors when both APIs fail`() {
        ...
        coEvery { auto24Service.findCarPrice(plateNumber) } returns
            CarPriceResult(price = null, error = "Vehicle not found", durationSeconds = 0.5)
        coEvery { veegoService.getTaxInfo(plateNumber) } returns VeegoResult.error("Not found", 0.2)
    ...
    fun `should handle auto24 exception gracefully`() {
        ...
        coEvery { auto24Service.findCarPrice(plateNumber) } throws RuntimeException("Failed to fetch price: timeout")
        coEvery { veegoService.getTaxInfo(plateNumber) } returns createSuccessfulVeegoResult()
    ...
    fun `should translate kuni to english in market price`() {
        val plateNumber = "876BCH"
        coEvery { auto24Service.findCarPrice(plateNumber) } returns CarPriceResult(price = "1000 € kuni 2000 €", durationSeconds = 1.0)
        coEvery { veegoService.getTaxInfo(plateNumber) } returns createSuccessfulVeegoResult()
    ...
    fun `should not include car header when make is null`() {
        val plateNumber = "876BCH"
        coEvery { auto24Service.findCarPrice(plateNumber) } returns CarPriceResult(price = "5000 €", durationSeconds = 1.0)
        coEvery { veegoService.getTaxInfo(plateNumber) } returns createVeegoResultWithoutMakeModel()
    ...
    fun `should call both services in parallel`() {
        ...
        coVerify(exactly = 1) { auto24Service.findCarPrice(plateNumber) }
        coVerify(exactly = 1) { veegoService.getTaxInfo(plateNumber) }
    }
