...
class OpenRouterClientTest {
  ...
  fun `appends online suffix to model id in request`() {
    ...
    client.classifyWithOnlineSearch(AiModel.GPT_6_LUNA, "prompt")

    expect(captured.captured.model).toEqual("openai/gpt-6-luna:online")
  }
  ...
  fun `returns content from response`() {
    ...
    val result = client.classifyWithOnlineSearch(AiModel.GPT_6_LUNA, "prompt")
  ...
  fun `returns null when api key blank`() {
    ...
    val result = blankKeyClient.classifyWithOnlineSearch(AiModel.GPT_6_LUNA, "prompt")
