...
**Every UI change is gated by a pixel harness.** The `🖼️ Visual Regression Tests` CI job captures each route at three viewports (390×844, 768×1024, 1440×900) at `maxDiffPixels: 0`, with baselines in `docs/superpowers/baseline/`, and every capture must stub its data routes so a shot never reads the live database. Any visual change is therefore a baseline change: work is not finished until its baselines are re-recorded deliberately, and an unexplained diff is a regression, not noise.

Screenshots are only comparable within one rendering environment, so both recording and verification run inside `mcr.microsoft.com/playwright:v1.62.1-noble` on `linux/arm64` — never against a host browser, and never under emulation, which silently collapses every `oklch()` colour to black. Verify with `npm run visual`, re-record with `npm run visual:update`; both start their own dev server and take Playwright arguments after `--`. Bumping `@playwright/test` means bumping the image tag in `package.json` and `.github/workflows/ci.yml` and re-recording every baseline in the same commit.
