...
interface FakeChartOptions {
  seriesId?: string
  visible?: boolean
  extremes?: { low: number; high: number } | null
}
...
const fakeChart = (
  texts: DrawnText[],
  { seriesId = 'totalValue', visible = true, extremes = { low: 0, high: 2 } }: FakeChartOptions
) =>
  ({
...
const draw = (options: FakeChartOptions = {}) => {
  ...
  ;(rangeExtremes.afterDatasetsDraw as any)?.(fakeChart(texts, options), {}, {})
  return texts
}
...
describe('rangeExtremes', () => {
  ...
  it('should draw nothing when the series has no extremes', () => {
    expect(draw({ extremes: null })).toEqual([])
  })

  it('should draw nothing for a performance series', () => {
    expect(draw({ seriesId: 'portfolio' })).toEqual([])
  })
})
