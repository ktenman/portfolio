    ...
    <div class="mb-3">
      <div class="d-flex flex-wrap gap-2 mb-2">
        <span v-for="id in selectedIds" :key="id" class="instrument-tag" :style="tagStyle(id)">
          {{ instrumentDetailMap.get(id)?.name ?? id }}
          <button
            type="button"
            class="tag-close"
            :style="{ color: tagTextColor(id) }"
            @click="removeInstrument(id)"
          >
            &times;
          </button>
        </span>
...
const instrumentDetailMap = computed(
  () => new Map(allInstruments.value.map(i => [i.id, { name: i.name, symbol: i.symbol }]))
)
...
const instrumentColorMap = computed(() => {
  ...
  if (data.value) {
    data.value.instruments.forEach((inst, i) => {
      map.set(inst.instrumentId, colors[i % colors.length])
    })
  }
  return map
})
